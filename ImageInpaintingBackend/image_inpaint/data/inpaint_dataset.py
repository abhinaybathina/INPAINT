...
class InpaintDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        masks = {mask_type: self.transforms_fun(self.read_mask(
            mask_paths[mask_type], mask_type))[:1, :, :] for mask_type in mask_paths}
    ...
    def read_mask(self, path, mask_type):
        ...
        if mask_type == 'random_bbox':
            ...
            for i in range(self.random_bbox_number):
                bbox = InpaintDataset.random_bbox(
                    self.resize_shape, self.random_bbox_margin, self.random_bbox_shape)
                bboxs.append(bbox)
    ...
    def read_bbox_xml(path):
        ...
        for obj in objects:
            ...
            bbox = [bndbox['ymin'], bndbox['xmin'], bndbox['ymax'] -
                    bndbox['ymin'], bndbox['xmax']-bndbox['xmin']]
            bndboxs.append(bbox)
        #print(bndboxs, size)
    ...
    def random_ff_mask(config):
        ...
        for i in range(num_v):
            ...
            for j in range(1+np.random.randint(5)):
                ...
                cv2.line(mask, (start_y, start_x),
                         (end_y, end_x), 1.0, brush_w)
...
class InpaintWithFileDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        masks = {mask_type: 255*self.transforms_fun(self.read_mask(
            mask_paths[mask_type], mask_type))[:1, :, :] for mask_type in mask_paths}
        # print(masks)
...
class NoriInpaintDataset(NoriBaseDataset):
    ...
    def __init__(self, img_nori_list_path, img_nori_path, mask_flist_paths_dict,
        ...
        self.img_nori_list, self.img_cls_ids, self.img_nr = self.initialize_nori(
            img_nori_list_path, img_nori_path)
    ...
    def read_mask(self, path, mask_type):
        ...
        if mask_type == 'random':
            ...
            bbox = InpaintDataset.read_bbox(path)
        mask = np.tile((InpaintDataset.bbox2mask(
            bbox, self.resize_shape)*255).astype(np.uint8), (1, 1, 3))
        # print(mask.shape)
        # print(mask)
    ...
    def __getitem__(self, index):
        ...
        img = self.transforms_fun(self.read_img(img_path))
        masks = {mask_type: self.transforms_fun(self.read_mask(
            mask_paths[mask_type], mask_type)) for mask_type in mask_paths}
